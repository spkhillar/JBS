...
import com.web.util.JqGridObjectMapper;
 ...
 * @author
 */
public abstract class AbstractJqGridFilterController extends BaseAuthenticatedController {
  ...
  public String getFilteredRecords(String filters, String sord, String sidx, Map<String, Object> paramObject,
    ...
    List<Rule> excludedRules =
        JqGridFilterQueryBuilder.getJpqlPredicate(jqgridFilter, execludedFilters, paramObject, predicateString,
          targetClass, groupOperator);
    List<String> excludedPredicates = getExcludedPredicates(excludedRules, paramObject);
    ...
    if (CollectionUtils.isNotEmpty(excludedPredicates)) {
      predicateSecondPart = StringUtils.join(excludedPredicates, groupOperator);
      if (StringUtils.isNotBlank(predicateFirstPart)) {
        finalPredicate = finalPredicate + groupOperator + predicateSecondPart;
      } else {
    ...
    if (orderByField != null) {
      finalPredicate = finalPredicate + " order by " + orderByField + " " + sord;
    }
   ...
   */
  public List<String> getExcludedPredicates(List<Rule> excludedRules, Map<String, Object> paramObject) {
    ...
    if (CollectionUtils.isNotEmpty(excludedRules)) {
      ...
      for (Rule rule : excludedRules) {
        String value = getFilterExcludedPropertyQueryMapping().get(rule.getField());
        if (StringUtils.isBlank(value)) {
          continue;
        }
        Rule newRule = new Rule(null, value, rule.getOp(), rule.getData());
        exeRule =
            JqGridFilterQueryBuilder.createRuleString(newRule, paramObject, paramSize,
              ServiceUtil.checkValueType(newRule.getData()));
  ...
  public String getOrderByfield(String filter) {
    ...
    String excludedPropMapping = this.getFilterExcludedPropertyOrderMapping().get(filter);
